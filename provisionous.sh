...
# Call function to load provision configuration
generic_load_provision_config $args_provisioner;
provisioner=${provision_info['provisioner']};
...
case $args_command in
  ...
  'cleanup' )
    ...
    if [ ! -n "$args_distro" ] && [ ! -n "$args_tag" ]; then
      ...
      generic_clean_untagged $provisioner;
    # Invalid arguments
    else
      # Call function to show error and quit
  ...
  'clean' )
    ...
    if [ -n "$args_distro" ] && [ -n "$args_tag" ]; then
      ...
      generic_clean_distro_tag $provisioner $args_distro $args_tag;
    # Distro provided, tag not provided
    elif [ -n "$args_distro" ] && [ ! -n "$args_tag" ]; then
      # Call function to clean all tags for a distro
      generic_clean_distro_tags $provisioner $args_distro;
    # Distro and tag not provided
    elif [ ! -n "$args_distro" ] && [ ! -n "$args_tag" ]; then
      # Call function to clean all tags for all distros
      generic_clean_distros $provisioner;
    # Invalid arguments
    else
      # Call function to show error and quit
      message_error 'Invalid arguments.';
  ...
  'pull' )
    ...
    if [ -n "$args_distro" ] && [ -n "$args_tag" ]; then
      ...
      generic_pull_distro_tag $provisioner $args_distro $args_tag;
    # Distro provided, tag not provided
    elif [ -n "$args_distro" ] && [ ! -n "$args_tag" ]; then
      # Call function to pull all tags for a distro
      generic_pull_distro_tags $provisioner $args_distro;
    # Distro and tag not provided
    elif [ ! -n "$args_distro" ] && [ ! -n "$args_tag" ]; then
      # Call function to pull all tags for all distros
      generic_pull_distros $provisioner;
    # Invalid arguments
    else
      # Call function to show error and quit
      message_error 'Invalid arguments.';
  ...
  'build' )
    ...
    if [ -n "$args_distro" ] && [ -n "$args_tag" ]; then
      ...
      generic_build_distro_tag $provisioner $args_distro $args_tag;
    # Distro provided, tag not provided
    elif [ -n "$args_distro" ] && [ ! -n "$args_tag" ]; then
      # Call function to build all tags for a distro
      generic_build_distro_tags $provisioner $args_distro;
    # Distro and tag not provided
    elif [ ! -n "$args_distro" ] && [ ! -n "$args_tag" ]; then
      # Call function to build all tags for all distros
      generic_build_distros $provisioner;
    # Invalid arguments
    else
      # Call function to show error and quit
      message_error 'Invalid arguments.';
  ...
  'run' )
    ...
    if [ -n "$args_distro" ] && [ -n "$args_tag" ]; then
      ...
      generic_run_distro_tag $provisioner $args_distro $args_tag;
    # Distro provided, tag not provided
    # Distro and tag not provided
    # Invalid arguments
    else
